/*
  ...
  1. Changes
    - Update admin_update_user_plan function to remove reference to non-existent "reason" column
    - Store reason only in plan_changes table, not in user_plans
*/

-- Drop the existing function first
DROP FUNCTION IF EXISTS admin_update_user_plan(uuid, text, text, text);

-- Create new version of the function without the reason parameter for user_plans
