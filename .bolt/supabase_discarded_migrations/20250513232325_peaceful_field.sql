-- First drop the existing get_user_plan function to avoid return type errors
DROP FUNCTION IF EXISTS get_user_plan(uuid);

-- Drop the existing admin_update_user_plan function
DROP FUNCTION IF EXISTS admin_update_user_plan(uuid, text, text, text);

-- Create updated function with proper subscription period validation and extra leverage
...
BEGIN
  ...
  IF p_reason LIKE '%Alavancagem Extra%' THEN
    -- Extract leverage value from reason (e.g., "Alavancagem Extra 50x")
    DECLARE
      v_leverage_match text;
      v_leverage_value int;
    BEGIN
      v_leverage_match := regexp_replace(p_reason, '.*Alavancagem Extra ([0-9]+)x.*', '\1');
      
      -- Check if we got a valid number
      IF v_leverage_match ~ '^[0-9]+$' THEN
        v_leverage_value := v_leverage_match::int;
        -- Store the leverage value
        v_subscription_data := jsonb_set(v_subscription_data, '{extra_leverage}', to_jsonb(v_leverage_value));
      END IF;
    END;
  END IF;
...
-- Create or replace function to get user plan with optional modules
...
BEGIN
  ...
  SELECT
    ...
    up.is_active,
    up.subscription_id,
    COALESCE(up.subscription_data, '{}'::jsonb),
    COALESCE(up.subscription_period, 'monthly')
  FROM
    user_plans up
  WHERE
...
-- Create a trigger function to refresh client state when plan changes
CREATE OR REPLACE FUNCTION refresh_client_state()
RETURNS TRIGGER AS $$
BEGIN
  -- Broadcast to all clients that a plan has changed
  PERFORM pg_notify('plan_changes', json_build_object(
    'user_id', NEW.user_id,
    'plan_type', NEW.plan_type,
    'subscription_period', NEW.subscription_period,
    'subscription_data', NEW.subscription_data
  )::text);
...
-- Create trigger on user_plans table
DROP TRIGGER IF EXISTS refresh_client_on_plan_change ON user_plans;
CREATE TRIGGER refresh_client_on_plan_change
AFTER INSERT ON user_plans
FOR EACH ROW
EXECUTE FUNCTION refresh_client_state();